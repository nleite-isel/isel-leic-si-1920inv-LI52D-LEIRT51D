...
//        return 1; // The right way
    } else {
       int err = X509_STORE_CTX_get_error(x509_ctx);
       printf("Verification failed: %s.\n",
                    X509_verify_cert_error_string(err)); 
//        return 0; // The right way
    }
...
SSL* setupTLSClient(const char* hostname)
{
   ...
   // Use in www.facebook.com, www.example.com
   SSL_CTX_load_verify_locations(ctx, NULL, "./cert"); // If you have a certification chain with 3 or more certificates
   // Use in bank32.com
//   SSL_CTX_load_verify_locations(ctx, "./cert/modelCA_cert.pem", NULL); // If you have a CA root + a leaf certificate


   // Step 2: Create a new SSL structure for a connection
   SSL* ssl = SSL_new (ctx);

   // Step 3: Enable the hostname check by calling verify_callback function
...
//    int port = 8080;
   ...
   // Test #1
//   char *hostname = "bank32.com";
//   int port = 8085;

   // Test #2 - Activate and deactivate verification
